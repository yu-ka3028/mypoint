...
| `weekly_routine_completions` | 本人のみ | 毎週ルーティンの完了履歴（1完了1レコード） |
...
| `weekly_routine` | 毎週ルーティン | **100pt ÷ Σ(weekly_count)**（スロット数で均等配分） |
| `urgent` | 突発緊急 | **ユーザー指定**（任意のpt） |
| `someday` | 突発いつかやる | **ユーザー指定**（任意のpt） |

毎週ルーティンは `weekly_count`（1〜6）でスロット数を指定。1スロット完了ごとに1レコード追加。

**例**:
- 毎日ルーティンを5個登録 → 各20pt（合計100pt）
- 掃除機(weekly_count=3) + 窓拭き(weekly_count=1) = 4スロット → 各25pt
- 突発タスク「確定申告」→ ユーザーが30ptと設定

タスク数・weekly_countが変わった時点で均等配分を再計算する。
...
### ランディングページ `/`（未ログイン時のみ表示）

ポイントカードを全面に出し、日常タスクをこなすとポイントが貯まっていくワクワク感を直感的に伝えるページ。

1. キャッチコピー（短いテキスト。「日常タスクをこなすだけでポイントが貯まる」趣旨）
2. ポイントカード アニメーションデモ（メインビジュアル）
   - タスク完了 → スタンプが押されて埋まっていくループアニメーション
   - サンプルデータで自動再生
3. Google ログインCTA
4. ランキング（user_profiles の points_total 降順 上位5名）

### ホーム `/`（ログイン済み）

タブ切り替えUI：

- **タスクタブ**（デフォルト表示）: タスク一覧・完了操作
- **ポイントカードタブ**: 実ポイントデータのポイントカード
...
タスク完了
 → 「+Xpt ポイントゲット！」トースト通知
 → ポイントカードタブにバッジ表示（未確認スタンプ数）
 → ユーザーがタブを開いた時にスタンプアニメーション再生
```

- 未確認スタンプ数は **localStorage** で管理（リロード後も保持、デバイス間同期なし）
- タブを開いた時点でlocalStorageをリセット・アニメーション再生
...
### Phase 4
- [ ] タスク完了時のトースト通知 + タブバッジ（localStorageで未確認スタンプ数管理）
- [ ] ポイントカードタブを開いた時のスタンプアニメーション
- [ ] レスポンシブデザイン調整
- [ ] Vercel デプロイ
